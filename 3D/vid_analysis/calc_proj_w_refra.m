...
function [coor_b,coor_s1,coor_s2] = calc_proj_w_refra(coor_3d,proj_params)
...
for n = 1:npts
    x = coor_3d(:,n);
    coor_b(1,n) = fa1p00+fa1p10*x(3)+fa1p01*x(1)+fa1p20*x(3)^2+fa1p11*x(3)*x(1)+fa1p30*x(3)^3+fa1p21*x(3)^2*x(1);
    coor_b(2,n) = fa2p00+fa2p10*x(3)+fa2p01*x(2)+fa2p20*x(3)^2+fa2p11*x(3)*x(2)+fa2p30*x(3)^3+fa2p21*x(3)^2*x(2);
    coor_s1(1,n) = fb1p00+fb1p10*x(1)+fb1p01*x(2)+fb1p20*x(1)^2+fb1p11*x(1)*x(2)+fb1p30*x(1)^3+fb1p21*x(1)^2*x(2);
    coor_s1(2,n) = fb2p00+fb2p10*x(1)+fb2p01*x(3)+fb2p20*x(1)^2+fb2p11*x(1)*x(3)+fb2p30*x(1)^3+fb2p21*x(1)^2*x(3);
    coor_s2(1,n) = fc1p00+fc1p10*x(2)+fc1p01*x(1)+fc1p20*x(2)^2+fc1p11*x(2)*x(1)+fc1p30*x(2)^3+fc1p21*x(2)^2*x(1);
    coor_s2(2,n) = fc2p00+fc2p10*x(2)+fc2p01*x(3)+fc2p20*x(2)^2+fc2p11*x(2)*x(3)+fc2p30*x(2)^3+fc2p21*x(2)^2*x(3);
end


% % 
% x = coor_3d;
% coor_b =  [fa1p00+fa1p10*x(3,:)+fa1p01*x(1,:)+fa1p20*x(3,:).^2+fa1p11*x(3,:).*x(1,:)+fa1p30*x(3,:).^3+fa1p21*x(3,:).^2.*x(1,:);
%     fa2p00+fa2p10*x(3,:)+fa2p01*x(2,:)+fa2p20*x(3,:).^2+fa2p11*x(3,:).*x(2,:)+fa2p30*x(3,:).^3+fa2p21*x(3,:).^2.*x(2,:)];
% coor_s1 = [fb1p00+fb1p10*x(1,:)+fb1p01*x(2,:)+fb1p20*x(1,:).^2+fb1p11*x(1,:).*x(2,:)+fb1p30*x(1,:).^3+fb1p21*x(1,:).^2.*x(2,:);
%     fb2p00+fb2p10*x(1,:)+fb2p01*x(3,:)+fb2p20*x(1,:).^2+fb2p11*x(1,:).*x(3,:)+fb2p30*x(1,:).^3+fb2p21*x(1,:).^2.*x(3,:)];
% coor_s2 = [fc1p00+fc1p10*x(2,:)+fc1p01*x(1,:)+fc1p20*x(2,:).^2+fc1p11*x(2,:).*x(1,:)+fc1p30*x(2,:).^3+fc1p21*x(2,:).^2.*x(1,:);
%     fc2p00+fc2p10*x(2,:)+fc2p01*x(3,:)+fc2p20*x(2,:).^2+fc2p11*x(2,:).*x(3,:)+fc2p30*x(2,:).^3+fc2p21*x(2,:).^2.*x(3,:)];
