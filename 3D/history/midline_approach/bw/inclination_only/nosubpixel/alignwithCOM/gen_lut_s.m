...
ballsize0 = [5,4.5,4,3.5,3,2.5,2,2,1.5,1];
% thickness of the sticks in the model
thickness0 = [4.5,4,3.5,3,2.5,2,2,1.5,1];

% distance between the fish and the camera
for depth = 1:5
    ballsize = ballsize0 * (0.85+0.05*depth);
    thickness = thickness0 * (0.85+0.05*depth);
    % length of a segment
    for nseglen = 1:30
        seglen = nseglen * 0.2;
        for n = 1:9
            ...
            for a = 1:360
                ...
                lut_bw_s{n,depth,nseglen,a} = bwmodel;
            end
        end
    end
end
