...
figure
hold on
errorbar(x,mean(t_prop_fs),e_t_fs,'color',bluem);
errorbar(x,mean(t_prop_slc),e_t_slc,'color',purplem);
errorbar(x,mean(t_prop_llc),e_t_llc,'color',goldm);
hold off
...
for n = 1:nswimbouts_fs
    ...
    plot(nc_fs_result{n,1}([1,ns/2+1,2,ns/2+2,3,ns/2+3]),'color',bluem,'DisplayName',sprintf('%d',n))
    hold on
end
...
for n = 1:nswimbouts_er
    ...
    if swimbouts_er(nc_er{n,1},5)<20
        ...
        plot(nc_er_result{n,1}([1,ns/2+1,2,ns/2+2,3,ns/2+3]),'color',purplem,'DisplayName',sprintf('%d',n))
        hold on
    else
        sig_llc = [sig_llc;nc_er_result{n,1}([1,ns/2+1,2,ns/2+2,3,ns/2+3])];
        plot(nc_er_result{n,1}([1,ns/2+1,2,ns/2+2,3,ns/2+3]),'color',goldm,'DisplayName',sprintf('%d',n))
        hold on
    end
end
...
figure
hold on
errorbar(x,mean(sig_fs),e_sig_fs,'color',bluem);
errorbar(x,mean(sig_slc),e_sig_slc,'color',purplem);
errorbar(x,mean(sig_llc),e_sig_llc,'color',goldm);
hold off
