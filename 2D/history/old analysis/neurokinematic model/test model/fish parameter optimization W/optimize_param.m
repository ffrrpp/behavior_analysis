...
A1 = [zeros(1,ns),ones(1,ns),zeros(1,ns),zeros(1,9)];
A2 = [zeros(1,ns),ones(1,ns),zeros(1,ns-1),1,zeros(1,9)];
A3 = [zeros(ns-1,ns),-[zeros(ns-1,1),diag(ones(ns-1,1))],[diag(ones(ns-1,1)),zeros(ns-1,1)]-[zeros(ns-1,1),diag(ones(ns-1,1))],zeros(ns-1,9)];
A = [A1;A2;A3];
...
% x0 = x00+noise/2;
xlb = [ones(1,ns),ones(1,ns)*5,ones(1,ns)*3,ones(1,9)];
xub = [ones(1,ns)*50,ones(1,ns*2)*50,ones(1,9)*50];
