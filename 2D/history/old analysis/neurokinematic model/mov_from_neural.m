for nn = 1:141    
    m = swimbouts_er_n(nn,1);
    i = swimbouts_er_n(nn,2);
    
    coor_er_mats = dir('C:\Users\Ruopei\Desktop\results\escape response\escape response with smoothing 20000');
    coor_er_matname = coor_er_mats(m+2).name;
    coor_mat_mf = importdata(['C:\Users\Ruopei\Desktop\results\escape response\escape response with smoothing 20000\' coor_er_matname]);
    ...
    [~,~,sign1] = find(x_ori(abs(x_ori)>0.4),1);
    nc_n = nc_er_neuro{nn};
    ...
    for n = 1:nframes
        ...
        c4 = cropcoor{n}(4);
        % x from neural model
        x_neuro = [x_all_mf{i}(n,1:3), 0, -deg2rad(diff(nc_er_neuro{nn,1}(n,:)))*sign(sign1*sign2)];
