...
% for n = 1:nfs
%     plot(tau_fs(n,:),'color',[gray,0.6],'DisplayName',sprintf('%d',n),'Linewidth',1.5)
% end
% for n = 1:nslc
%     plot(tau_slc(n,:),'color',[blue,0.6],'DisplayName',sprintf('%d',n),'Linewidth',1.5)
% end
% for n = 1:nllc
%     plot(tau_llc(n,:),'color',[pink,0.6],'DisplayName',sprintf('%d',n),'Linewidth',1.5)
% end
% 
% errorbar(x,nanmean(tau_fs),e_tau_fs,'color',gray,'Linewidth',6);
% errorbar(x,nanmean(tau_slc),e_tau_slc,'color',blue,'Linewidth',6);
% errorbar(x,nanmean(tau_llc),e_tau_llc,'color',pink,'Linewidth',6);
% hold off
% ax = gca;
% ax.LineWidth = 5;
% ax.FontSize = 30;
