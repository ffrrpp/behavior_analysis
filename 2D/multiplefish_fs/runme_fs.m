%% process free swimming videos

% location of video files
filepath = 'C:\Users\ruopei\Desktop\fs';
...
% process videos, detect and analyze swim bouts
for m = 1:22
    ...
    fish_in_vid = tracking_fs(v);
    goodswimbouts = classify_swimbouts_fs(fish_in_vid,v);
        ...
        for n = 1:nframes
            ...
            while err_ct >= 0
                try
                    ...
                    x(n,:) = x_cropped(n,:);
                    x(n,1) = x(n,1) + cropcoor{n}(3) - 1;
                    x(n,2) = x(n,2) + cropcoor{n}(1) - 1;
                    err_ct = -1;
                catch
                    err_ct = err_ct + 1;
                end
    ...
    results = struct('x_all',{x_all},'fval_all',{fval_all},...
        'fish_in_vid',{fish_in_vid},'goodswimbouts',goodswimbouts);
