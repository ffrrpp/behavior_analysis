...
function [diff,diff_tailtip1,diff_tailtip2,shape_panelty,oversize_panelty] = calc_diff_gray_detail(im,x,lut_head,lut_tail,seglen)
% global x_iter
% global diff_iter
...
bodypix = imblank;
% 
% if pt(1,9)<1 || pt(1,9)>imageSizeY || pt(2,9)<1 || pt(2,9)>imageSizeX
%     diff = 999999;
% else
    ...
    oversize_panelty = max(0,34000 - sum(graymodel(:)));
    diff = diff_gi + sum(sum(bodypix - im)) +...
        diff_tailtip1*2 + diff_tailtip2*1+shape_panelty+oversize_panelty;
% end
